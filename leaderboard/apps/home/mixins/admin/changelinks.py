...
def underscore_to_capitalize(string: 'str'):
    return string.split('__')[-1].replace('_', ' ').capitalize()
...
class AdminChangeLinksMixin(_Base):
    model: 'Type[Model]'
    change_links = []
    changelist_links = []
    ...
    def _add_change_link(self, model_field_name, admin_field_name, options):
        ...
        self._add_admin_field(
            admin_field_name, make_change_link(model_field_name, options))

    def _get_change_link(self, instance, model_field_name: 'str', admin_field_name, options):

        target_instance: 'Model' = functools.reduce(
            getattr, model_field_name.split('__'), instance)
    ...
    def _add_changelist_link_fields(self):
        field_config = parse_field_config(self.changelist_links)
        for model_field_name, admin_field_name, options in field_config:
            self._add_changelist_link(
                model_field_name, admin_field_name, options)
    ...
    def _add_changelist_link(self, model_field_name, admin_field_name, options):
        ...
        self._add_admin_field(
            admin_field_name, make_changelist_link(model_field_name, options))
    ...
    def _get_changelist_link(self, instance, model_field_name, options):
        ...
        def get_lookup_filter():
            return (options.get('lookup_filter') or
                    instance._meta.get_field(model_field_name).field.name)
    ...
    def decorate_link_func(self, func, model_field_name: 'str', options):

        func.short_description = options.get(
            'label') or underscore_to_capitalize(model_field_name)

        if options.get('admin_order_field'):
            func.admin_order_field = options['admin_order_field']
        else:
            try:
                field_names = model_field_name.split('__')

                field = self.model._meta.get_field(field_names[0])
                for field_name in field_names[1:]:
                    field = field.related_model._meta.get_field(
                        field_name)
